  ...
  if(external) {
    ...
    for(let possible_match of sitemap_urls) {
      if(possible_match.url == url) {
        ...
        possible_match.crawl_status = 'finished';
        found_link = true;
      }
    }
    if(!found_link) {
      sitemap_urls.push({url:url,status:status,crawl_status:'finished'});
    }
...
const crawl_neon_url = async function(url) {
  var starttime = new Date().getTime();
  ...
  if(status == 200) {
    ...
    let page_info = await scrape.getInfo({url:url,skip_marking:true,prefetched_meta:meta,neon:true});
...
const findNextCrawlableUrl = async function(randomize, filter) {
  ...
  console.log(stats.fin, 'of',stats.total,'links finished');
  return found_link;
}
